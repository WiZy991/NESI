...
export default function Header() {
	...
	useEffect(() => {
		const handleOpenMoreMenu = (e?: Event) => {
			clientLogger.debug('Получен запрос на открытие меню "Ещё" из онбординга', { event: e?.type })
		...
		const fetchNotifications = async () => {
			try {
				...
				if (res.ok) {
					...
					clientLogger.error('Ошибка уведомлений', undefined, { response: data })
					setNotifications([])
				}
			} catch (err) {
		...
		const updateActivity = async () => {
			try {
				...
				if (!res.ok) {
					...
					clientLogger.error('Ошибка обновления активности', undefined, { status: res.status, errorText })
				} else {
			...
			eventSource.onmessage = event => {
				try {
					...
					if (data.type === 'onlineCount') {
						clientLogger.debug('Обновление онлайн счетчика через SSE', { count: data.count })
	...
	const showNotification = useCallback(
		(data: any) => {
			clientLogger.debug('showNotification вызвана', { type: data.type, id: data.id })
			...
			if (shownNotificationsRef.current.has(notificationKey)) {
				clientLogger.debug('Пропускаем дубликат уведомления', { notificationKey })
				return
			}
			...
			if (isInChatsPage && isCurrentChatNotification) {
				clientLogger.debug('Пользователь в открытом чате, пропускаем уведомление')
				return
			}
			...
			if (typeof window !== 'undefined' && 'Notification' in window) {
				if (Notification.permission === 'granted') {
					try {
						...
						clientLogger.debug('Десктопное уведомление показано', { title: notificationTitle })
					} catch (error) {
			...
			if (
				...
				clientLogger.debug('Показываем модальный попап для достижения', { badgeName: data.badgeName })
				setAchievementBadge({
					id: data.badgeId,
					name: data.badgeName,
					icon: data.badgeIcon,
			...
			if (data.type !== 'login') {
				...
				clientLogger.debug('Добавление toast уведомления', { type: toastNotification.type, id: toastNotification.id })
				setToastNotifications(prev => {
					...
					if (existingToast) {
						clientLogger.debug('Toast уведомление уже существует, не добавляем', { toastId: existingToast.id })
						return prev
					}

					const newNotifications = [...prev, toastNotification]
					clientLogger.debug('Текущие toast уведомления', { count: newNotifications.length })
	...
	useEffect(() => {
		...
		const fetchUnreadMessages = async () => {
			try {
				...
				if (!text || text.trim() === '') {
					clientLogger.warn('Пустой ответ от API непрочитанных сообщений', { status: res.status })
				...
				if (res.ok) {
					...
					clientLogger.error('Ошибка получения непрочитанных сообщений', undefined, {
						status: res.status,
		...
		const connectSSE = () => {
			...
			clientLogger.debug('Подключение к SSE', { tokenPrefix: token.substring(0, 10) })
			...
			eventSource.onmessage = event => {
				try {
					...
					clientLogger.debug('Обработка уведомления', { type: data.type, id: data.id })
			...
			eventSource.onerror = error => {
				clientLogger.error('Ошибка SSE подключения', error, { readyState: eventSource.readyState })
				...
				clientLogger.warn('Количество ошибок SSE', { count: sseFailCountRef.current })
	...
	const handleNotificationClick = async (notif: any) => {
		...
		markAllRead().catch(err => clientLogger.error('Ошибка при отметке уведомлений', err))
	...
	const handleToastClose = (id: string) => {
		...
		setToastNotifications(prev => {
			...
			clientLogger.debug('Осталось toast уведомлений', { count: filtered.length })
			return filtered
		})
	}
