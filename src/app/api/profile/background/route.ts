...
import { getBackgroundById, isBackgroundUnlocked } from '@/lib/level/profileBackgrounds'
...
export async function GET(req: NextRequest) {
  try {
    ...
    try {
      ...
      logger.debug('Не удалось получить фон профиля из БД, используем дефолтное значение', { 
        userId: user.id,
...
export async function PATCH(req: NextRequest) {
  try {
    ...
    try {
      ...
      // Если поле profileBackground еще не существует в БД (миграция не применена)
      if (dbError.message?.includes('profileBackground') || dbError.code === 'P2021' || dbError.code === 'P2002') {
        logger.warn('Поле profileBackground не найдено в БД или миграция не применена', { 
          userId: user.id,
          error: dbError.message,
          code: dbError.code 
        })
        ...
        return NextResponse.json({ 
          success: true, 
          backgroundId,
          warning: 'Фон не сохранен в БД. Примените миграцию для полной функциональности.'
        })
      }
      throw dbError
    }
