...
export async function POST(req: NextRequest) {
	try {
		...
		// В примере документации используется короткий идентификатор (например, "asdasdad")
		// Максимальный размер ограничен - используем короткий идентификатор
		// Если есть телефон - используем его (в формате +7XXXXXXXXXX), иначе - короткий хеш от user.id
		...
		if (userPhone) {
			...
			// Проверяем формат: +7 + 10 цифр (всего 12 символов)
			// Согласно документации: "+79606747611" - это +7 + 10 цифр
			if (/^\+7\d{10}$/.test(formattedPhone)) {
				...
				// Если формат неверный, используем короткий хеш от user.id
				// Используем первые 8 символов user.id для короткого идентификатора
				paymentRecipientId = user.id.substring(0, 8)
			}
		} else {
			// Если телефона нет, используем короткий идентификатор на основе user.id
			// Используем первые 8 символов user.id (как в примере документации "asdasdad" - 8 символов)
			paymentRecipientId = user.id.substring(0, 8)
		}
