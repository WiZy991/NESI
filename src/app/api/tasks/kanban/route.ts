...
export async function PATCH(req: NextRequest) {
	...
	if (user.role !== 'customer') {
		return NextResponse.json({ error: 'Доступ только для заказчиков' }, { status: 403 })
	}
		...
		await prisma.$transaction(
			sanitized.map(update =>
				prisma.task.update({
					where: { id: update.id },
					data: {
						kanbanColumn: update.column,
						kanbanOrder: update.order,
					},
				})
			)
		)
	...
	await logActivity(user.id, 'kanban_update', req, {
		updates: sanitized,
	})
