			...
			if (addCardResult.ErrorCode === '204') {
				...
				return NextResponse.json(
					{ 
						...
						details: `❌ Привязка карты временно недоступна.\n\n` +
							`Проблема: Метод AddCard требует основной терминал интернет-эквайринга с правильным паролем.\n\n` +
							`Проверьте:\n` +
							`1. Пароль для основного терминала ${TBANK_CONFIG.TERMINAL_KEY} в личном кабинете Т-Банка\n` +
							`2. Убедитесь, что используется пароль от основного терминала, а не от E2C терминала\n` +
							`3. Если основного терминала нет, обратитесь в Т-Банк для получения отдельного терминала интернет-эквайринга\n\n` +
							`Примечание: E2C терминал работает для выплат, но не для привязки карт.`,
					},
					{ status: 400 }
				)
			}
