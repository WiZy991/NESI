...
export async function POST(req: NextRequest) {
	try {
		...
		// КРИТИЧНО: Для владельца платформы проверяем статус платежа по сделке
		// Комиссия может быть выведена только если платеж заказчика подтвержден (CONFIRMED)
		if (isPlatformOwner) {
			try {
				// Находим PaymentId по DealId из транзакций пополнения заказчика
				...
				if (depositTx?.paymentId) {
					const { TBankClient } = await import('@/lib/tbank/client')
					const tbankClient = new TBankClient()
					
					// Проверяем статус платежа
					const paymentStatus = await tbankClient.getPaymentState(depositTx.paymentId)
					
					console.log('🔍 [CREATE-WITHDRAWAL] Статус платежа для вывода комиссии:', {
						dealId: finalDealId,
						paymentId: depositTx.paymentId,
						status: paymentStatus.Status,
						success: paymentStatus.Success,
						isPlatformOwner: true,
					})

					// Если платеж не подтвержден, пытаемся подтвердить его
					if (paymentStatus.Success && paymentStatus.Status && paymentStatus.Status !== 'CONFIRMED') {
						console.log('🔄 [CREATE-WITHDRAWAL] Платеж не подтвержден, пытаемся подтвердить для вывода комиссии:', {
							paymentId: depositTx.paymentId,
							currentStatus: paymentStatus.Status,
						})

						const confirmResult = await tbankClient.confirmPayment(depositTx.paymentId)
						
						if (confirmResult.Success && confirmResult.Status === 'CONFIRMED') {
							console.log('✅ [CREATE-WITHDRAWAL] Платеж успешно подтвержден для вывода комиссии:', {
								paymentId: depositTx.paymentId,
								dealId: finalDealId,
							})
						} else {
							console.warn('⚠️ [CREATE-WITHDRAWAL] Не удалось подтвердить платеж для вывода комиссии:', {
								paymentId: depositTx.paymentId,
							...
							// Если платеж не подтвержден, возвращаем ошибку
							if (confirmResult.Status !== 'CONFIRMED') {
								return NextResponse.json(
									{
										error: `❌ Платеж по сделке не подтвержден. Комиссия не может быть выведена.\n\n` +
											`• DealId: ${finalDealId}\n` +
											`• PaymentId: ${depositTx.paymentId}\n` +
											`• Статус: ${confirmResult.Status || paymentStatus.Status}\n\n` +
											`💡 Решение:\n` +
											`• Платеж должен быть подтвержден (статус CONFIRMED) перед выводом комиссии\n` +
											`• Попробуйте вывести средства через несколько минут\n` +
											`• Или обратитесь в поддержку Т-Банка (acq_help@tbank.ru)`,
									},
									{ status: 400 }
								)
							}
						...
						console.log('✅ [CREATE-WITHDRAWAL] Платеж уже подтвержден, можно выводить комиссию:', {
							paymentId: depositTx.paymentId,
							dealId: finalDealId,
						})
					} else if (!paymentStatus.Success) {
						console.warn('⚠️ [CREATE-WITHDRAWAL] Не удалось проверить статус платежа для вывода комиссии:', {
							paymentId: depositTx.paymentId,
					...
					console.warn('⚠️ [CREATE-WITHDRAWAL] PaymentId не найден для DealId при выводе комиссии:', {
						dealId: finalDealId,
						note: 'Не удалось проверить статус платежа',
					})
				}
			} catch (checkError: any) {
				console.error('❌ [CREATE-WITHDRAWAL] Ошибка проверки статуса платежа для вывода комиссии:', checkError)
				// Не прерываем выполнение - продолжаем попытку выплаты
				// Возможно, платеж уже подтвержден или подтвердится автоматически
			}
		}
