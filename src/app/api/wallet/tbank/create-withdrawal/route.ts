...
export async function POST(req: NextRequest) {
	try {
		...
		// КРИТИЧНО: Блокируем вывод на новую карту ДО всех проверок
		// Вывод на новую карту требует RSA сертификат, который не настроен
		if (hasCardData && !hasCardId) {
			console.log('🚫 [CREATE-WITHDRAWAL] БЛОКИРОВКА: Попытка вывода на новую карту (не поддерживается)')
			return NextResponse.json(
				{
					error: 'Вывод на новую карту временно недоступен. Используйте привязанную карту или СБП',
				},
				{ status: 400 }
			)
		}
			...
			if (!finalDealId) {
				...
				if (!diagnosticInfo.hasTBankDeposits) {
					if (isPlatformOwner) {
						...
						errorMessage += 'Сначала пополните баланс, затем попробуйте вывести средства.'
					}
		...
		if (!finalDealId) {
			return NextResponse.json(
				{
					error: 'Не удалось обработать запрос. Сначала пополните баланс, затем попробуйте вывести средства.',
				},
				{ status: 400 }
			)
		}
		...
		try {
			...
			if (!withdrawal.Success) {
				...
				logger.error('Create withdrawal failed', undefined, { errorCode: withdrawal.ErrorCode })
		...
		if (!withdrawal || !withdrawal.PaymentId) {
			...
			return NextResponse.json(
				{
					error: 'Ошибка при создании выплаты. Попробуйте позже',
				},
				{ status: 500 }
			)
		}
