						...
						<CardContent className='p-5'>
							...
							<div className='grid grid-cols-2 gap-4 mb-4'>
								...
								<MiniStat
									...
									value={`${Number(
										stats.subcategoriesStats._avg.minPrice || 0
									).toFixed(0)} ₽`}
								/>
							</div>
					...
					<Card className='bg-black/60 border border-emerald-500/30 shadow-[0_0_15px_rgba(16,185,129,0.3)]'>
						<CardContent className='p-5'>
							...
							<ResponsiveContainer width='100%' height={250}>
								<LineChart
									data={mergeGrowth(stats.usersByDay, stats.tasksByDay)}
								>
									<CartesianGrid strokeDasharray='3 3' stroke='#14532d' />
...
function mergeGrowth(users: any[], tasks: any[]) {
	...
	users?.forEach(
		(u: any) =>
			(map[u.date] = { ...map[u.date], date: u.date, users: Number(u.count) })
	)
	tasks?.forEach(
		(t: any) =>
			(map[t.date] = { ...map[t.date], date: t.date, tasks: Number(t.count) })
	)
