		...
		<div className='text-white'>
			...
			<div className='grid grid-cols-2 gap-4 bg-gray-900 p-4 rounded-lg border border-gray-800'>
				<div>
					...
					<p>
						<span className='text-gray-400'>Имя:</span> {user.fullName || '—'}
					</p>
					<p>
						<span className='text-gray-400'>Роль:</span> {user.role}
					</p>
					<p>
				...
				<div>
					...
					<p>
						<span className='text-gray-400'>XP:</span> {user.xp}
					</p>
					<p>
						<span className='text-gray-400'>Рейтинг:</span>{' '}
						{user.avgRating ? Number(user.avgRating).toFixed(2) : '—'}
					</p>
					<p>
						<span className='text-gray-400'>Выполнено задач:</span>{' '}
						...
						<tbody>
							{user.transactions.map(tr => (
								<tr key={tr.id} className='border-t border-gray-800'>
									...
									<td
										className={`p-2 ${
											Number(tr.amount) > 0 ? 'text-green-400' : 'text-red-400'
										}`}
									>
