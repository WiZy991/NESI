...
export async function sendAdminAlert(
	...
	try {
		...
		if (!adminId) {
			console.warn(
				'⚠️ PLATFORM_OWNER_ID не настроен, уведомление админу не отправлено'
			)
			return
		}
...
export async function validateWithdrawal(
	...
	if (isNewAccount && amount > 5000) {
		return {
			allowed: false,
			error:
				'Новые аккаунты (младше 7 дней) могут выводить максимум 5000₽ за раз',
		}
	}
 ...
 */
export async function detectCircularDeals(
	userAId: string,
	userBId: string
): Promise<{
	count: number
