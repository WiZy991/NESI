...
export async function createWithdrawal(
	...
	// Согласно документации: "CardData для выплаты по зашифрованным данным карты"
	// "Token используется для CardId, когда данные хранятся на стороне банка"
	// Проверяем params.cardData, чтобы определить, используется ли CardData
	if (params.cardData) {
		console.log('🔐 [TBANK] CardData используется - Token НЕ передается (используется подпись по сертификату RSA):', {
			hasCardData: !!params.cardData,
			hasCustomerKey: !!params.customerKey,
			note: 'CardData требует подписи по сертификату RSA, а не Token. Token НЕ должен быть в запросе.',
		})
		// Явно удаляем Token из requestBody, если он был добавлен ранее
	...
	// ФИНАЛЬНАЯ ПРОВЕРКА: Убеждаемся, что Token НЕ передается вместе с CardData
	if (requestBody.CardData && requestBody.Token) {
		console.error('❌ [TBANK] КРИТИЧЕСКАЯ ОШИБКА: Token присутствует вместе с CardData!', {
			hasCardData: !!requestBody.CardData,
			hasToken: !!requestBody.Token,
			note: 'Token НЕ должен передаваться вместе с CardData. CardData требует подписи по сертификату RSA.',
		})
		// Явно удаляем Token
		delete requestBody.Token
	}
	...
	if (!data.Success && data.ErrorCode !== '0') {
		...
		if (data.ErrorCode === '107' && params.cardData) {
			errorMessage = 
				'❌ Вывод на новую карту через E2C терминал недоступен.\n\n' +
				'E2C терминал Т-Банка не поддерживает CardData без RSA шифрования.\n\n' +
				'Для вывода средств доступны следующие способы:\n\n' +
				...
				'💡 Рекомендация: Используйте вывод через СБП - это самый быстрый и удобный способ.'
		} else {
