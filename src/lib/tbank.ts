...
export async function createWithdrawal(
	...
	if (!data.Success && data.ErrorCode !== '0') {
		...
		if (data.ErrorCode === '648') {
			...
			if (data.Details.includes('wrong.payout.amount')) {
				...
				errorMessage = `❌ Неверная сумма выплаты (${amountInKopecks / 100} ₽).${dealIdInfo}\n\n` +
					`🔍 Возможные причины (согласно документации Т-Банка):\n` +
					`• Сумма превышает доступный баланс сделки\n` +
					`• В сделке нет успешных операций покупки (платежей в статусе CONFIRMED)\n` +
					`• Платежи по сделке еще не подтверждены (не был вызван метод Confirm)\n` +
					`• Сделка закрыта или не найдена\n` +
					`• Сумма слишком мала (минимум для СБП: 10 ₽, минимум для карты: 1 ₽)\n` +
					`• Баланс сделки был изменен после последней проверки\n` +
					`• По сделке уже были произведены выплаты, превышающие баланс\n\n` +
					`💡 Решение:\n` +
					`• Убедитесь, что платежи по сделке имеют статус CONFIRMED\n` +
					`• Проверьте доступный баланс сделки в личном кабинете Т-Банка\n` +
					`• Уменьшите сумму выплаты до доступного баланса сделки\n` +
					`• Если это первая выплата, убедитесь, что был создан успешный платеж (Init → Confirm)\n` +
					`• Проверьте, что DealId корректный и сделка активна`
		...
		// Для ошибок СБП не блокируем полностью - возможно это временная проблема
		if (data.Message) {
			...
			if (messageLower.includes('сбп недоступен') || messageLower.includes('сбп не доступен') || messageLower.includes('недоступен для магазина') || messageLower.includes('сбп не поддерживается')) {
				// СБП недоступен - возвращаем понятную ошибку, но не блокируем другие способы выплат
				errorMessage = `❌ Способ выплаты СБП недоступен для вашего терминала.\n\n` +
					...
					`Пока СБП недоступен, используйте вывод на банковскую карту (с привязанной картой через CardId или с RSA ключом).`
			}
		}
		
		// Обработка ошибок для выплат на карту
		// Проверяем как Details, так и Message для ошибок связанных с CardId/CardData
		...
		if (errorText.includes('cardid') || errorText.includes('carddata') || errorText.includes('привязан') || errorText.includes('привяз')) {
			const originalError = data.Details || data.Message || 'Неверные параметры'
			errorMessage = `❌ Ошибка при выплате на карту: ${originalError}\n\n` +
