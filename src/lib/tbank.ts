...
export async function createWithdrawal(
	...
	// - CardData можно передавать в Init для выплат
	// - Подпись запроса — через Token (не RSA)
	// - CardData требует шифрования RSA для самих данных карты (PAN, ExpDate, CVV, CardHolder)
	// - Но подпись запроса генерируется через Token (SHA-256 от параметров + Password)
	...
	if (params.cardData) {
		...
		console.log('✅ [TBANK] Используется CardData для выплаты:', {
			hasCardData: !!requestBody.CardData,
			hasCustomerKey: !!requestBody.CustomerKey,
			note: 'CardData передается в Init, подпись запроса через Token (не RSA)',
		})
	}
	...
	// - CardData для выплаты по зашифрованным данным карты
	// - Token используется для подписи запроса (для CardId, CardData или СБП)
	// 
	// Условная обязательность передачи:
	// - CardId для выплаты на привязанную карту (с Token)
	// - или CardData для выплаты по зашифрованным данным карты (с Token)
	// - или Phone + SbpMemberId для выплаты по СБП (с Token)
	
	const usesCardId = !!(params.cardId || requestBody.CardId)
	const usesCardData = !!(params.cardData || requestBody.CardData)
	
	// Всегда генерируем Token (для CardId, CardData или СБП)
	// Token используется для подписи запроса
	// CardData НЕ участвует в расчете Token (исключается в generateToken)
	if (usesCardId || usesCardData) {
	...
	console.log('📤 [TBANK] Подготовка запроса на выплату:', {
		...
		note: requestBody.CardData 
			? 'CardData используется - подпись запроса через Token (не RSA)'
			: requestBody.CardId
