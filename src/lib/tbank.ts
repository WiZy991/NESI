...
export interface CreateWithdrawalParams {
	...
	cardData?: string // Данные карты (зашифрованные RSA) - подпись запроса через Token
	customerKey?: string // CustomerKey - используется с cardData для автоматической привязки карты
...
export async function createWithdrawal(
	...
	// CardData используется для выплаты по зашифрованным данным карты
	// 
	// КРИТИЧНО: Согласно документации Т-Банка:
	// - CardId используется с Token (данные хранятся на стороне банка)
	// - CardData используется с подписью по сертификату RSA (НЕ с Token!)
	// - Token используется ТОЛЬКО для CardId, НЕ используется для CardData
	// - CardData требует шифрования RSA для самих данных карты (PAN, ExpDate, CVV, CardHolder)
	// - CardData требует подписи запроса по сертификату RSA (DigestValue, SignatureValue, X509SerialNumber)
	// - Привязка карты, переданной в CardData, происходит автоматически к CustomerKey (если передан)
	if (params.cardData) {
		...
		throw new Error(
			...
			'• Token используется ТОЛЬКО для CardId (когда данные хранятся на стороне банка)\n' +
			'• CardData требует:\n' +
			'  1. Шифрования данных карты открытым ключом RSA (X509 RSA 2048)\n' +
			'  2. Подписи запроса по сертификату RSA (DigestValue, SignatureValue, X509SerialNumber)\n\n' +
			...
			'   • Используется Token для подписи запроса\n\n' +
			'💡 Рекомендация: Используйте вывод через СБП или привяжите карту заранее через AddCard.'
		)
	}
	...
	// - CardData для выплаты по зашифрованным данным карты (требует RSA сертификат)
	// - Token используется ТОЛЬКО для CardId (когда данные хранятся на стороне банка)
	// - CardData НЕ используется с Token, требует подписи по сертификату RSA
	// 
	// Условная обязательность передачи:
	// - CardId для выплаты на привязанную карту (с Token)
	// - или CardData для выплаты по зашифрованным данным карты (с RSA сертификатом, НЕ с Token)
	// - или Phone + SbpMemberId для выплаты по СБП (с Token)
	
	const usesCardId = !!(params.cardId || requestBody.CardId)
	
	// Token используется ТОЛЬКО для CardId или СБП
	// CardData требует подписи по сертификату RSA (проверка выполнена выше)
	if (usesCardId) {
