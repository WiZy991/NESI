# ⚙️ Настройка для сервера nesi.su

## 🎯 Конфигурация для вашего сервера

### 1. Переменные окружения (.env)

Создайте файл `.env` на сервере в папке `NESI/`:

```bash
# === Основные настройки ===
DATABASE_URL="postgresql://user:password@localhost:5432/nesi"
JWT_SECRET="ваш_jwt_секрет"

# === URL сайта ===
NEXT_PUBLIC_BASE_URL="https://nesi.su"

# === Т-Банк Мультирасчеты ===

# Терминал для приема платежей (от покупателей)
TBANK_TERMINAL_KEY="TinkoffBankTest"
TBANK_TERMINAL_PASSWORD="ваш_пароль_от_тбанка"

# E2C терминал для выплат (исполнителям)
TBANK_E2C_TERMINAL_KEY="TerminalKeyE2C"
TBANK_E2C_TERMINAL_PASSWORD="ваш_e2c_пароль"

# Режим: test для тестирования, production для боевого режима
TBANK_MODE="test"
```

**⚠️ Важно:**

- Замените все пароли на реальные от Т-Банка
- Для production смените `TBANK_MODE="production"`

---

## 🌐 Настройка Т-Банка для nesi.su

### Notification URL (webhook)

В личном кабинете [business.tbank.ru](https://business.tbank.ru):

```
https://nesi.su/api/tbank/webhook
```

### Success/Fail URLs

```
Success URL: https://nesi.su/profile?payment=success
Fail URL: https://nesi.su/profile?payment=failed
```

---

## 🔒 Безопасность

### IP Whitelist

Напишите на `acq_help@tbank.ru` с просьбой добавить IP вашего сервера nesi.su в whitelist.

### Firewall

Разрешите входящие запросы от IP Т-Банка на порт 443 (HTTPS).

**IP адреса Т-Банка:**

```
91.194.226.0/23
91.218.132.0/22
212.233.80.0/22
```

**Команды для UFW (Ubuntu):**

```bash
# Добавить правила для webhook от Т-Банка
sudo ufw allow from 91.194.226.0/23 to any port 443 proto tcp comment 'TBank Webhook 1'
sudo ufw allow from 91.218.132.0/22 to any port 443 proto tcp comment 'TBank Webhook 2'
sudo ufw allow from 212.233.80.0/22 to any port 443 proto tcp comment 'TBank Webhook 3'

# Перезагрузить firewall
sudo ufw reload

# Проверить что добавлено
sudo ufw status numbered | grep TBank
```

**Важно:** Эти правила НЕ блокируют остальной трафик, они ДОПОЛНИТЕЛЬНО разрешают IP Т-Банка.

**Подробнее:** См. файлы `НАСТРОЙКА_FIREWALL_UBUNTU.md` и `FIREWALL_КОМАНДЫ.txt`

---

## 🚀 Деплой на сервер

### Шаг 1: Загрузить код

```bash
# На сервере nesi.su
cd /path/to/nesi
git pull origin main
```

### Шаг 2: Установить зависимости

```bash
cd NESI
npm install
```

### Шаг 3: Применить миграцию

```bash
npx prisma migrate deploy
npx prisma generate
```

**⚠️ Используйте `migrate deploy` на production, а не `migrate dev`!**

### Шаг 4: Собрать проект

```bash
npm run build
```

### Шаг 5: Перезапустить

```bash
# Если используете PM2:
pm2 restart nesi

# Если используете systemd:
sudo systemctl restart nesi

# Или просто:
npm start
```

---

## ✅ Проверка работоспособности

### 1. Проверьте, что сайт доступен

```bash
curl https://nesi.su
# Должен вернуть HTML страницу
```

### 2. Проверьте endpoint статуса

```bash
curl https://nesi.su/api/tbank/status
```

Должен вернуть:

```json
{
	"configured": true,
	"missing": [],
	"mode": "test",
	"baseUrl": "https://rest-api-test.tinkoff.ru"
}
```

### 3. Проверьте webhook endpoint

```bash
curl -X POST https://nesi.su/api/tbank/webhook \
  -H "Content-Type: application/json" \
  -d '{"test": "ping"}'
```

Должен вернуть `OK` (даже с неверными данными, т.к. обработчик возвращает OK в любом случае)

### 4. Тестовое пополнение

1. Откройте `https://nesi.su/profile`
2. Вкладка "Кошелек"
3. "Пополнить баланс"
4. Введите 100 ₽
5. Должен произойти редирект на форму Т-Банка

---

## 🐛 Troubleshooting для nesi.su

### Ошибка "Cannot connect to database"

Проверьте DATABASE_URL:

```bash
# На сервере проверьте подключение к PostgreSQL
psql -U user -d nesi -c "SELECT 1;"
```

### Webhook не приходит

1. Проверьте, что `https://nesi.su/api/tbank/webhook` доступен извне
2. Проверьте firewall (порт 443 открыт?)
3. Проверьте логи сервера:

```bash
# PM2 логи
pm2 logs nesi

# Или системные логи
tail -f /var/log/nginx/error.log
```

### Ошибка SSL/HTTPS

Убедитесь, что SSL сертификат для nesi.su установлен и действителен:

```bash
# Проверка сертификата
curl -vI https://nesi.su 2>&1 | grep -i "SSL\|certificate"
```

---

## 📋 Чеклист для nesi.su

- [ ] Код загружен на сервер
- [ ] `.env` создан с правильными переменными
- [ ] `NEXT_PUBLIC_BASE_URL="https://nesi.su"`
- [ ] Миграция применена (`npx prisma migrate deploy`)
- [ ] Проект собран (`npm run build`)
- [ ] Сервис перезапущен
- [ ] Webhook URL настроен: `https://nesi.su/api/tbank/webhook`
- [ ] Success URL: `https://nesi.su/profile?payment=success`
- [ ] Fail URL: `https://nesi.su/profile?payment=failed`
- [ ] IP сервера добавлен в whitelist Т-Банка
- [ ] IP Т-Банка разрешены в firewall
- [ ] SSL сертификат активен
- [ ] Протестировано пополнение
- [ ] Протестирован вывод

---

## 🔑 Получение учетных данных Т-Банка

### Для nesi.su нужно:

1. **Подать заявку** на [tbank.ru/kassa](https://tbank.ru/kassa)
2. **Указать в заявке:**

   - Сайт: `https://nesi.su`
   - Продукт: Мультирасчеты
   - Email для связи
   - ИНН/ОГРН (если юр.лицо)

3. **Получите данные:**

   - TerminalKey (для платежей)
   - Password
   - TerminalKeyE2C (для выплат)
   - PasswordE2C

4. **Внесите в `.env`** на сервере

---

## 📊 Мониторинг на nesi.su

### Логи

```bash
# Смотреть логи приложения
pm2 logs nesi --lines 100

# Смотреть только ошибки
pm2 logs nesi --err

# Логи Т-Банка (grep)
pm2 logs nesi | grep "TBank"
```

### База данных

```bash
# Подключиться к БД
psql -U user -d nesi

# Проверить сделки
SELECT id, "spAccumulationId", status, "totalAmount", "createdAt"
FROM "TBankDeal"
ORDER BY "createdAt" DESC
LIMIT 5;

# Проверить платежи
SELECT "paymentId", amount, status, "createdAt"
FROM "TBankPayment"
ORDER BY "createdAt" DESC
LIMIT 10;
```

---

## 🎯 Production готовность для nesi.su

### После тестирования:

1. Смените режим:

```bash
# В .env
TBANK_MODE="production"
```

2. Получите боевые учетные данные от Т-Банка

3. Обновите `.env` с боевыми ключами

4. Перезапустите сервис

5. Протестируйте на небольших суммах!

---

## 📞 Контакты поддержки

**Т-Банк:**

- 📧 acq_help@tbank.ru
- 🌐 developer.tbank.ru
- 💼 business.tbank.ru

**Ваш сайт:** https://nesi.su  
**Webhook:** https://nesi.su/api/tbank/webhook  
**Профиль:** https://nesi.su/profile

---

✅ **Готово к работе на nesi.su!**
